...
export function renderProductsTable() {
    ...
    window.productsData.forEach(product => {
        ...
        availabilitySpan.textContent = '…'; // Изначально нет данных
        ...
        deliveryDateSpan.textContent = '…'; // Изначально нет данных
        ...
        priceSpan.textContent = product.base_price ? `${product.base_price.toFixed(2)} руб.` : 'Нет цены';
        ...
        retailPriceSpan.textContent = product.retail_price ? `${product.retail_price.toFixed(2)} руб.` : 'Нет розничной цены';
    ...
    // Загрузить остатки и даты доставки
    if (window.productsData.length) {
        const ids = window.productsData.map(p => p.product_id);
        loadAvailability(ids);
    }

    // colResizable destroy+init (ТОЛЬКО здесь, не в main.js!)
    try {
        const $table = $('#productTable');
        if ($table.length > 0 && $.fn.colResizable) {
        ...
        console.error('Ошибка инициализации colResizable:', e);
    }
}